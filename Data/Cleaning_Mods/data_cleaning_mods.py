...
def train_and_pred(using,topic,clf):
...
def get_results():
  ...
  for topic in tqdm(top_ten_topics):
      model_results = {}
      for clf_idx, clf in enumerate(clfs,clf_names):
          clf_name = clf_names[clf_idx]
          test_label, predictions = train_and_pred(using, topic,clf)
...
def download_results(results):
    ...
    for name, group in grouped_data:
        ...
        urllib.request.urlretrieve(csv_link, csv_link)
